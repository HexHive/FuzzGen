...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    /* * * function pool #1 * * */
    //{
        // initializing argument 'val_HcZ'
        int32_t val_HcZ_0 = 0;
        int32_t *val_HcZ_1 = &val_HcZ_0;

        gsm_option(dep_6, 1, val_HcZ_1); /* vertex #1 */
    ...
    /* * * function pool #2 * * */
    //{
        // initializing argument 'c_gXI'
        for (uint64_t i_0=0; i_0<33; ++i_0) {
            c_gXI[i_0] = E.eat1();
        }

        // initializing argument 'target_Ein'
        for (uint64_t i_0=0; i_0<76; ++i_0) {
            target_Ein[i_0] = 0 /* WO */;
        }

        gsm_explode(dep_6, (uint8_t *)&c_gXI, (int16_t *)&target_Ein); /* vertex #4 */
